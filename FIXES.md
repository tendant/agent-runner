...
### [C17] Configured git token silently loses to an inherited system/global credential.helper for the same host
- **Files:** `internal/template/memory_git.go` (`configureCredHelper`, MEMORY_GIT_TOKEN), `internal/executor/workspace.go` (now-exported `ConfigureCredHelper`, GIT_TOKEN for agent workspaces), `internal/api/command.go` (`handleRepoAdd`, `handleRepoUpdate` — GIT_TOKEN for `/repo add`/`/repo update`, previously a *third*, independent copy of the same vulnerable pattern)
- **Problem:** All three call sites configured the token via a plain `git config credential.helper <helper>` (or, for `/repo add`, an unguarded one-shot append), which only appends to the repo's own `--local` config. It does not suppress a system- or global-level `credential.helper` that already has a cached credential for the same host (macOS's `osxkeychain` is on by default via Xcode's git and needs no explicit config to be active). Git tries helpers in file-read order (system, global, local) and stops at the first one that returns a full username+password, so a stale cached credential for a *completely unrelated* project on the same host silently wins over the token agent-runner was explicitly configured to use — with no error, just an auth failure under the wrong identity (or, worse, a silent success as the wrong identity). `handleRepoUpdate` additionally had a "skip reconfiguring if something is already set" check that a naive read (`git config credential.helper`, not `--get-all`) satisfies just as easily from an inherited entry as from a correctly-configured one, masking the problem instead of catching it.
- **How it was found:** live-validating TESTPLAN.md §2/§9 against a real Gitea remote (`git.memochat.ai`) with real HTTPS token credentials. `/memory push` and `/repo update` both failed with `Repository not found` against a repo that demonstrably existed and was accessible — traced to `git credential fill` returning a stale macOS Keychain entry for the same host from an unrelated prior project, not the token agent-runner had just configured.
- **Fix:** `ConfigureCredHelper` (exported from `internal/executor`, now the single shared implementation) writes an empty `credential.helper` value before the real one — a git-documented idiom that resets the accumulated helper chain up to that point, so only the helper set here applies. `memory_git.go`'s `configureCredHelper` does the same for `MEMORY_GIT_TOKEN`. `command.go`'s `handleRepoAdd`/`handleRepoUpdate` now delegate to `executor.ConfigureCredHelper` instead of duplicating the (previously vulnerable) logic; `handleRepoUpdate` always reconfigures unconditionally rather than skip-checking.
- **Status:** [x] done — regression test `TestConfigureCredHelper_OverridesInheritedHelper` in `internal/executor/workspace_test.go` (verified it fails against the pre-fix code and passes against the fix, using `GIT_CONFIG_GLOBAL` to simulate an inherited decoy credential without touching the real system config). Live-confirmed end-to-end against `git.memochat.ai`: `/memory push`, `/memory pull`, `/repo add`, and `/repo update` (the one that originally exposed this) all now authenticate correctly in the presence of an unrelated cached credential for the same host.
